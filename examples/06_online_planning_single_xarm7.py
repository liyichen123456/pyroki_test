...
def main():
    ...
    base = server.scene.add_frame("/robot")
    ...
    ik_target_handle = server.scene.add_transform_controls(
        "/ik_target", scale=0.2, position=(0.25, 0.0, 0.2), wxyz=(0, 0, 1, 0)
    )
    ...
    while True:
        ...
        sol_traj, sol_pos, sol_wxyz = pks.solve_online_planning(
            ...
            target_position=np.array(ik_target_handle.position - offset),
        ...
        if hasattr(target_frame_handle, "batched_positions"):
            target_frame_handle.batched_positions = np.array(sol_pos) + offset  # type: ignore[attr-defined]
            target_frame_handle.batched_wxyzs = np.array(sol_wxyz)  # type: ignore[attr-defined]
        else:
            # This is an older version of Viser.
            target_frame_handle.positions_batched = np.array(sol_pos) + offset  # type: ignore[attr-defined]
